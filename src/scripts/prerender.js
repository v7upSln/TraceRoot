...
// Load indexed mods from reports_cache if present
let indexedMods = [];
...
if (fs.existsSync(cachePath)) {
  try {
    ...
    indexedMods = Object.values(rawCache);
    console.log(`[*] Loaded ${indexedMods.length} indexed mods for static pre-rendering.`);
  } catch (err) {
    console.warn(`[!] Failed to parse _mods_index.json: ${err.message}`);
  }
} else {
  console.warn(`[!] _mods_index.json not found at ${cachePath}`);
}
